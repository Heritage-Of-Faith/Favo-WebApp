...
| `transitionOrder(id, toState)` | Server action | barista | State machine: ordered→in_progress→ready→collected. `in_progress` deducts stock (P2). `ready` fires Web Push + accrues loyalty + pg_notify. |

...
type EditState = { id: string | null; opensAt: string; closesAt: string } | null;
...
export default function TodaySessionsPlanner({ initialSessions, todayLabel, fallbackLabel }: TodaySessionsPlannerProps) {
  ...
  async function save() {
    ...
    setBusy(true);
    setError(null);
    const input = { opensAt: edit.opensAt, closesAt: edit.closesAt || null };
    const res = edit.id
  ...
  return (
    <section aria-label="Today's hours" className="mb-8 rounded-md border border-border-subtle bg-elevated p-4">
            ...
            <div key={s.id} className="flex flex-wrap items-center gap-2 rounded border border-border-subtle bg-surface px-3 py-2">
              ...
              <span className="ml-auto flex gap-3">
                <button type="button" disabled={busy}
                  onClick={() => setEdit({ id: s.id, opensAt: s.opensAt, closesAt: s.closesAt ?? "" })}
      ...
      {!edit && (
        <button type="button" disabled={busy} onClick={() => setEdit({ id: null, opensAt: "", closesAt: "" })}
      ...
      <p className="mt-3 text-xs text-text-muted">
        ...
        changes don&rsquo;t notify customers — only the POS opening prompt does.
      </p>
    </section>
  );
}

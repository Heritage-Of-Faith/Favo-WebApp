...
describe("draftOrder store", () => {
  ...
  it("setCustomer stores and retrieves a customer", () => {
    const c = { id: "c1", name: "Louis", phone: "0821234567", email: null, loyaltyPoints: 50 };
    store().setCustomer(c);
    expect(store().customer).toEqual(c);
  });

  it("setCustomer(null) clears the customer", () => {
    const c = { id: "c1", name: "Louis", phone: null, email: null, loyaltyPoints: 0 };
    store().setCustomer(c);
  ...
  it("reset returns to initial state", () => {
    store().addItem(item1);
    store().setCustomer({ id: "c1", name: "Louis", phone: null, email: null, loyaltyPoints: 0 });
